...
/**
 * Step-by-step Find-Minimum: keep walking left (mirrors HeritageBST.find_min())
 */
export function solveBSTFindMin(root: BSTNode | null): BSTStep[] {
  const steps: BSTStep[] = [];
  const visited: string[] = [];
  ...
  steps.push({
    currentId: null,
    visitedIds: [],
    description: `Start Find-Minimum at the root. The minimum key in a BST is always the leftmost node.`,
  });
  ...
  while (current) {
    ...
    if (current.left) {
      steps.push({
        currentId: current.id,
        visitedIds: [...visited],
        description: `"${current.place.name}" has a left child. Continue moving left.`,
        comparison: 'left',
      });
      current = current.left;
    } else {
      steps.push({
        currentId: current.id,
        visitedIds: [...visited],
        description: `"${current.place.name}" has no left child. This is the minimum (alphabetically first) site.`,
        comparison: 'found',
        result: current.place,
      });
      current = null;
...
/**
 * Step-by-step Find-Maximum: keep walking right (mirrors HeritageBST.find_max())
 */
export function solveBSTFindMax(root: BSTNode | null): BSTStep[] {
  const steps: BSTStep[] = [];
  const visited: string[] = [];
  ...
  steps.push({
    currentId: null,
    visitedIds: [],
    description: `Start Find-Maximum at the root. The maximum key in a BST is always the rightmost node.`,
  });
  ...
  while (current) {
    ...
    if (current.right) {
      steps.push({
        currentId: current.id,
        visitedIds: [...visited],
        description: `"${current.place.name}" has a right child. Continue moving right.`,
        comparison: 'right',
      });
      current = current.right;
    } else {
      steps.push({
        currentId: current.id,
        visitedIds: [...visited],
        description: `"${current.place.name}" has no right child. This is the maximum (alphabetically last) site.`,
        comparison: 'found',
        result: current.place,
      });
      current = null;
